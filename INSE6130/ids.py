...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("/ids_app/logs/ids.log"),
    ],
)
    ...
    while True:
        try:
            for proc in psutil.process_iter(
                ...
                # Detect processes running as root that shouldn't be
                if (
...
def monitor_process_creations():
    ...
    Logs all new processes except the IDS itself.
    """
    ...
    while True:
        try:
            ...
            logging.error(f"Error in process creation monitoring: {e}")


class FileMonitorHandler(FileSystemEventHandler):
    """
    Monitors file system events.
    Detects modifications to critical files.
    """

    def on_any_event(self, event):
        excluded_dirs = ["/var/log", "/ids_app/logs"]
        ...
        if not event.is_directory:
            critical_paths = ["/etc/passwd", "/etc/shadow", "/etc/hosts", "/etc/group"]
            normalized_critical_paths = [
                os.path.realpath(path) for path in critical_paths
            ]

            if event_src_path in normalized_critical_paths:
                alert_message = f"Critical file {event.event_type}: {event_src_path}"
                logging.warning(alert_message)
...
def monitor_container_escape_attempts():
    ...
    while True:
        try:
            # Check for unexpected mounts or access to sensitive host paths
            for path in sensitive_host_paths:
                if os.path.exists(path):
                    alert_message = (
                        f"Potential container escape attempt detected: Access to {path}"
                    )
                    logging.warning(alert_message)
            time.sleep(5)
        except Exception as e:
            logging.error(f"Error in container escape monitoring: {e}")
